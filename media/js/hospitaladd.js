	...
	$.ajax({
        ...
        success: function (data) {
            ...
            if($("#options").val() == "Service"){
                ...
                var cost1 = parseInt($("#service_cost").val()) * (10**18)
                transaction1 = operations_contract.methods.addServices(hosp_id, name1, desc1, cost1.toString(), img_hash)
                txn1 = send(transaction1)
            }
            else{
                var name2 = $("#medicine_name").val()
                var desc2 = $("#medicine_desc").val()
                var cost2 = parseInt($("#medicine_cost").val()) * (10**18)
                transaction2 = operations_contract.methods.addMedicines(hosp_id, name2, desc2, cost2.toString(), img_hash)
